...
try {
    ...
    # FastClaw may have left a Windows reserved-name `nul` file in the install
    # tree; normal Remove-Item can't delete it. Sweep those via the \\?\ device
    # path first so the recursive delete below doesn't throw.
    if (Test-Path $TestInstallDir) {
        $fcDir = Join-Path $TestInstallDir "resources\fastclaw"
        foreach ($reserved in @("nul", "con", "aux", "prn")) {
            $rp = Join-Path $fcDir $reserved
            if (Test-Path -LiteralPath $rp) {
                try { [System.IO.File]::Delete("\\?\" + $rp); Write-Host "  removed reserved file: $rp" } catch {}
            }
        }
