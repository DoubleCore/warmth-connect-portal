...
import { ArrowLeft, CornerDownLeft, Loader2, Plus, Sparkles } from "lucide-react";
      ...
      <div className="mx-auto w-full max-w-4xl px-8 py-10">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <h1 className="mt-2 text-3xl font-semibold tracking-tight">{t("research.title")}</h1>
            <p className="mt-1 text-sm text-muted-foreground">{t("research.subtitle")}</p>
          </div>
          {/*
           * 新建会话：把 useCommandStream 里缓存的 sessionId 丢掉，transcript 清空，
           * 下一次 run() 会重新 POST /api/command/sessions，Backend 侧的
           * conversation_history 也从空开始。busy 态禁用，避免半途打断还在跑的流。
           */}
          <button
            type="button"
            ...
            className="inline-flex shrink-0 items-center gap-1.5 rounded-lg border border-border bg-card px-3 py-2 text-xs font-medium text-muted-foreground transition-colors hover:border-primary/40 hover:text-foreground disabled:cursor-not-allowed disabled:opacity-50"
          >
        ...
        {isEmpty ? (
          <div className="mt-10 rounded-2xl border border-dashed border-border bg-card/40 px-6 py-12 text-center">
            ...
            <p className="mt-2 text-sm text-muted-foreground">{t("research.empty.hint")}</p>
            <Link
              to="/"
